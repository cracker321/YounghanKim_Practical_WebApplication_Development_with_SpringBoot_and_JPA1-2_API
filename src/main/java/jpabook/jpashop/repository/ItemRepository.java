...
//================================================================================================================
    ...
    public void save(Item item){ //- '클라이언트로부터 받아온 Item 객체'를 'DB에 저장만 하고 리턴값을 따로 반환해주지 않아도
                                 //   되기에('저장'만 하는 것이니깐 당연함..), 이 메소드의 자료형은 void임!

        if(item.getId() == null){ //- '클라이언트로부터 받아온 현재 Item 객체'는 당연히 'JPA에 저장되기 전까지 그 id값이 없기
            ...
            em.merge(item); //만약, 클라이언트로부터 받아온 Item 객체가 이미 id값이 있는 상태라면(이미 예전에 영속화되어 DB에
                            //저장되어 이미 id값이 있는 상태라면 - 즉, 기존 DB 속 어떤 자료를 꺼내와서 사용자가 그것을 수정하기 희망),
                            //DB에 저장되어 있는 기존의 그 Item 객체를 강제로 '수정(업데이트)'한다는 뜻임.
        }
    }
